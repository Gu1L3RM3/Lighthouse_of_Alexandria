...
class CurrentSourceItem(Item):
    def __init__(self, x, y, active,props):
        ...
        self.img =  ResourceManager.get().load_image("eletric_components/current_source.png",size=(32,32))
        ...
        self.add(
            ...
            Sprite(self.img),
            LabelComponent(f'I',self.value,FONT,font_size=10,offset_y=16),
            AlwaysOnTop(),
            LightComponent()
        )
    def on_active(self):
    ...
    def on_collect(self, entity):
        if not isinstance(entity,Player):
            return
        self.em.post(events={'type':'current_source_collected','value':self.value})
