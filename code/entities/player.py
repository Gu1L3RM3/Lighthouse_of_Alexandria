...
class Player(Entity):
    def __init__(self, x: float = 100, y: float = 100):
        ...
        pos = Position(x, y)
        vel = Velocity(0, 0)
        col = Collider(10, 6,offset_x=2,offset_y=13)
    ...
    def _set_animation(self, new_state: str):
        """Define uma nova animação somente se ela for diferente da atual."""
    ...
    def _get_dir_name(self, direction: Vector2) -> str:
        """Converte vetor de direção em nome de animação (front, back, left, right)."""
