...
class SerializationManager:
    ...
    def load_eletric_storage()->dict:
        try:
            with open('eletric_storage.json', 'r') as f:
                eletric_storage_data = json.load(f)
            
            print(f"Eletric Storage carregado de {'eletric_storage.json'}")
            return eletric_storage_data
        except FileNotFoundError:
            raise FileNotFoundError(f"Arquivo de salvamento não encontrado: {'eletric_storage.json'}")
    ...
    def save_eletric_storage(data: dict):
        """Salva os dados do estoque elétrico em um arquivo JSON."""
        try:
            with open('eletric_storage.json', 'w') as f:
                json.dump(data, f, indent=4)
            print(f"Estoque elétrico salvo em {'eletric_storage.json'}")
        except Exception as e:
            print(f"Erro ao salvar o estoque elétrico: {e}")
    @staticmethod
