...
def smna(net_list):
    ...
    def parse_eng_value(s_val: str) -> float:
        """Converte uma string com notação de engenharia para float."""
        s_val = s_val.lower()
        multipliers = {
            'p': 1e-12, 'n': 1e-9, 'u': 1e-6, 'm': 1e-3,
            'k': 1e3, 'meg': 1e6, 'g': 1e9, 't': 1e12
        }
        # Verifica se o final da string corresponde a algum multiplicador
        for suffix, multiplier in multipliers.items():
            if s_val.endswith(suffix):
                # Remove o sufixo e multiplica o número
                numeric_part = s_val[:-len(suffix)]
                return float(numeric_part) * multiplier
        
        # Se nenhum sufixo for encontrado, apenas converte para float
        return float(s_val)
    ...
    def indep_source(line_nu):
        ...
        df.loc[line_nu,'value'] = parse_eng_value(tk[3])
    ...
    def vccs_sub_network(line_nu):
        ...
        df.loc[line_nu,'value'] = parse_eng_value(tk[5])
    ...
    def cccs_sub_network(line_nu):
        ...
        df.loc[line_nu,'value'] = parse_eng_value(tk[4])
